...
namespace KPostProcessing
{
    ...
    public class BloomGraphDrawer
    {
        ...
        public void Prepare(float thresholdLinear,float softKnee,float intensity)
        {
            _rangeX = 6;
            _rangeY = 1.5f;

            _threshold = thresholdLinear;
            _knee = softKnee * _threshold + 1e-5f;

            // Intensity is capped to prevent sampling errors.
            _intensity = Mathf.Min(intensity, 10);
        }
